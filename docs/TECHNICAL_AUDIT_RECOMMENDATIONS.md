...
- **Текущее состояние:** в рабочем дереве определение уже имеет external linkage;
  исходная ошибка clang устранена. Добавлена GNU Make host-цель `test-host`; она собирает
  allocator и выполняет тесты в строгом C99-режиме. Цели `debug` и `release`
  собирают host-варианты, `target-debug` и `target-release` предназначены для
  `tricore-gcc`, а для host-отладки предусмотрена необязательная цель
  `test-sanitize` с AddressSanitizer и UndefinedBehaviorSanitizer.
...
**Текущее состояние:** исправлено во втором Critical-пункте: `memory_free`
проверяет `NULL` до вычисления адреса заголовка.
...
**Текущее состояние:** исправлено во втором Critical-пункте. Предикаты
переполнения покрывают округление запроса, сложение header/payload, split и
coalescing; переполняющий запрос отклоняется без изменения heap.
